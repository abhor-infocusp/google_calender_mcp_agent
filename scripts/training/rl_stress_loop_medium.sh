...
RUN_DIR="runs/rl_stress_qwen25_15b"
mkdir -p "${RUN_DIR}/logs/debug"

# Patch G configuration
export ART_DEADLOCK_TIMEOUT_S="${ART_DEADLOCK_TIMEOUT_S:-30}"
...
while true; do
    ...
    setsid /home/abhor/miniconda3/envs/agentic/bin/python scripts/training/rl_train_stress_medium.py \
        >> "$LOG" 2>&1 &
    PY_PID=$!
    CURRENT_PGID=$PY_PID
    wait $PY_PID
