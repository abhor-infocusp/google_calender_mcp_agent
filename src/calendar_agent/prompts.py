...
IMPORTANT: Only include events that a real person would actually put on their calendar. Do NOT include:
- Daily routines (morning routine, breakfast, lunch break, bedtime, wake up, sleep, wind down)
- Vague filler blocks (relax, free time, commute, prepare for bed)
These are NOT calendar events. Focus on meetings, appointments, tasks, social plans, and activities.
''')
...
**Calendar dates:** The week starts on Monday $monday_date. Tuesday is the next day, Wednesday the day after, and so on through Sunday.

**Available tools the agent has:**
- get_current_time: returns the current date/time and day of week
- list_events(time_min, time_max): list events, optionally filtered by time range
- get_event(event_id): get a single event by ID
- create_event(summary, start, end, attendees, description): create a new event
- update_event(event_id, updates): update an existing event's fields
- delete_event(event_id): delete an event
- respond_to_event(event_id, attending): RSVP to an event (ACCEPT/DECLINE/MAYBE)

**Hard constraints — every query MUST follow these:**
1. Only reference days and events within the Monday-to-Sunday calendar range. Never reference "next week", "last week", or dates outside this week.
2. Every query must be answerable using ONLY the tools listed above. Do NOT generate queries that ask for reminders, recurring events, repeating events, or notifications — these features do not exist. The query text itself must not contain words like "remind", "every day", "every week", "recurring", or "repeat".
3. The expected_behavior must describe concrete tool actions (e.g., "find the event and update its start time to 2 PM"), not vague outcomes.
...
[
  {
    ...
    "current_time": YYYY-MM-DDTHH:MM:SS # The simulated current time when the user asks the query. MUST use dates from the calendar week (Monday=$monday_date through Sunday). For "today" queries: use the addressed day. For "tomorrow" queries: use the day BEFORE the addressed day. For "yesterday" queries: use the day AFTER. Otherwise: use the Sunday before Monday (the day before $monday_date).
  }
]
''')
